...
angular.module('eqlytics', []).
  ...
  service('eqlytics', function() {
    ...
    this.logOnly = function() {
      var argsToJson = function(args) {
          try {
            return angular.toJson(args);
          } catch(e) {
            console.warn('[eqlytics] Could not transform arguments to JSON, logging as objects instead.');
            return args;
          }
      };

      var createLogOnlyFunction = function (functionName) {
        return function () {
          var args = Array.prototype.slice.call(arguments, 0, arguments.length);
          console.log('[eqlytics]', functionName, argsToJson(args));
        };
      };
      this.identify = createLogOnlyFunction('identify');
      this.track = createLogOnlyFunction('track');
      this.pageview = createLogOnlyFunction('pageview');
      this.alias = createLogOnlyFunction('alias');
    };
